"""
XGBoost regression using one or more input CSV files provided at runtime.

Usage (single file):
    python pavement_xgboost_v3.py Data_Texas_No_Missing.csv

Usage (multiple files - train/evaluate each separately):
    python pavement_xgboost_v3.py Data_Texas_No_Missing.csv Data_Alabama_No_Missing.csv

Usage (multiple files - merge all into one dataset):
    python pavement_xgboost_v3.py Data_Texas_No_Missing.csv Data_Alabama_No_Missing.csv --merge
...
def train_and_evaluate(work: pd.DataFrame, label: str) -> None:
    ...
    for i, name in enumerate(FEATURE_COLS):
        ...
        print(f"    {name:20s}: {gain.get(key, 0.0):.6f}")
    print()
