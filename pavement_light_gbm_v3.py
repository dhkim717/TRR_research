"""
LightGBM regression using one or more input CSV files provided at runtime.

Usage (single file):
    python pavement_light_gbm_v3.py Data_Texas_No_Missing.csv

Usage (multiple files - train/evaluate each separately):
    python pavement_light_gbm_v3.py Data_Texas_No_Missing.csv Data_Alabama_No_Missing.csv

Usage (multiple files - merge all into one dataset):
    python pavement_light_gbm_v3.py Data_Texas_No_Missing.csv Data_Alabama_No_Missing.csv --merge
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and evaluate a LightGBM regression model using pavement CSV file(s)."
    )
    parser.add_argument(
        "csv_files",
...
def train_and_evaluate(work: pd.DataFrame, label: str) -> None:
    ...
    # Parameter starting from XGBoost equivalents
