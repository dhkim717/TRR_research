"""
XGBoost regression using data.csv
...
# -----------------------------
# 1) Parse command-line argument and load CSV
# -----------------------------
parser = argparse.ArgumentParser(
    description="Run XGBoost regression for pavement IRI prediction."
)
parser.add_argument(
    "csv_file",
    help="Input CSV file name, e.g., Data_Texas_No_Missing.csv",
)
...
# -----------------------------
# Match paper-style constraints: max_depth=4, random_state=42
model = XGBRegressor(
    max_depth=4,
    random_state=42,
    objective="reg:squarederror",
    n_estimators=300,     # reasonable default; tune if needed
    learning_rate=0.05,   # reasonable default; tune if needed
    subsample=1.0,
...
# -----------------------------
# Print importances in the same order as features
gain = model.get_booster().get_score(importance_type="gain")

print("\nFeature importance (gain):")
for i, name in enumerate(feature_cols):
    key = f"f{i}"  # XGBoost internal feature name
    print(f"  {name:18s}: {gain.get(key, 0.0):.6f}")